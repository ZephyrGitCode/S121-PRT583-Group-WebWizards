...
<?php foreach($list as $blah){
  ...
  <h1 style="text-align:center;"  id= "leaderboard_heading"> <?php echo"Yearly Leaderboard - $years"?></h1>
...
if (isset($_POST["submit"])) {
	$str = $_POST["search"];
	$sth = $con->prepare("SELECT *,SUM(score.score)AS totalscore FROM USER,score WHERE user.fname=score.Username AND score.year = YEAR(CURRENT_TIMESTAMP()) AND user.fname LIKE'%$str%' GROUP BY fname");
